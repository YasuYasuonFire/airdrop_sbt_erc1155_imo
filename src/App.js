...
const CONTRACT_ADDRESS = "0xf8C20a3A6774aDd1629367A03dcE71101cFA15a8";
...
const setupEventListener = async () => {
  ...
  const askContractToMintNft = async (event) => {
    event.preventDefault();//submitイベントは停止する
    console.log("askContractToMintNft");
    try {
      const { ethereum } = window;
      if (ethereum) {
        const provider = new ethers.providers.Web3Provider(ethereum);
        ...
        let nftTxn = await connectedContract.makeAnEpicNFT({
          value:ethers.utils.parseEther(payvalue)//★payable対応
        });
        ...
        <div className="header-container">
          ...
          {currentAccount === "" ? (
            ...
            <form onSubmit={askContractToMintNft}>
              <div>
              <p className="sub-text">send over 0.01ETH</p><br></br>
                <input
                  value = {payvalue}
                  onChange={(event) => setPayvalue(event.target.value)}
                />
              </div>
              <p className="sub-text">{payvalue}</p>
              <br></br>
